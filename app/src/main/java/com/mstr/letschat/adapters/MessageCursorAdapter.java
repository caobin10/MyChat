...
import com.mstr.letschat.views.FileMessageView;
...
//		}
        ...
        } else if (ChatMessageTableHelper.isFileMessage(cursor)) {
            bindFile(context, (FileMessageView) view, cursor);
        }
...
//	}
    ...
    private void bindFile(Context context, FileMessageView view, Cursor cursor) {
        long x = cursor.getColumnIndex(ChatMessageTable.COLUMN_NAME_MESSAGE);
        String filename = cursor.getString(cursor.getColumnIndex(ChatMessageTable.COLUMN_NAME_MESSAGE));
        String filePath = cursor.getString(cursor.getColumnIndex(ChatMessageTable.COLUMN_NAME_MEDIA_URL));
        int start = filePath.lastIndexOf(".");
        String s = filePath.substring(start + 1, filePath.length());
        view.setFileText(filename + "." + s);
    }
