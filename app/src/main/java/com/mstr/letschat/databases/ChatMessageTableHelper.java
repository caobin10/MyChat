...
public class ChatMessageTableHelper {
    ...
    public static final int TYPE_INCOMING_FILE = 7;
    public static final int TYPE_OUTGOING_FILE = 8;
...
//    }
    ...
    public static ContentValues newFileMessage(String jid,String body,long timeMillis,String path,boolean outgoing){
        ContentValues values = new ContentValues();
        values.put(ChatMessageTable.COLUMN_NAME_JID,jid);
        values.put(ChatMessageTable.COLUMN_NAME_MESSAGE,body);
        values.put(ChatMessageTable.COLUMN_NAME_TIME,timeMillis);
        values.put(ChatMessageTable.COLUMN_NAME_TYPE,outgoing ? TYPE_OUTGOING_FILE : TYPE_INCOMING_FILE);
        values.put(ChatMessageTable.COLUMN_NAME_STATUS, outgoing ? STATUS_PENDING : STATUS_SUCCESS);
        values.put(ChatMessageTable.COLUMN_NAME_MEDIA_URL, path);
        return values;
    }
    ...
    public static boolean isFileMessage(int type){
        return type == TYPE_INCOMING_FILE || type == TYPE_OUTGOING_FILE;
    }
    ...
    public static boolean isFileMessage(Cursor cursor){
        int type = cursor.getInt(cursor.getColumnIndex(ChatMessageTable.COLUMN_NAME_TYPE));
        return isFileMessage(type);
    }
